...
for i in range(len(data)):
    ...
    # pairs2.append(qa_pairs)
    qa_pair = []
    question = str(df.iloc[i]['title']).lower()
    answer = str(df.iloc[i]['text']).lower()
    
    qa_pair.append(question.split()[:max_len])  # Tokenize & truncate question
    qa_pair.append(answer.split()[:max_len])    # Tokenize & truncate answer
    
    pairs2.append(qa_pair)
...
def collate_fn(batch):
    questions, answers = zip(*batch)  # unzip batch of tuples
    ...
    # Pad sequences
    questions_padded = pad_sequence([q[:max_len] for q in questions], batch_first=True, padding_value=0)
    answers_padded = pad_sequence([a[:max_len] for a in answers], batch_first=True, padding_value=0)

    return questions_padded, answers_padded

train_loader = torch.utils.data.DataLoader(Dataset(), batch_size=16, collate_fn=collate_fn)
...
class Embeddings(nn.Module):
    ...
    def __init__(self, vocab_size, d_model, max_len=128, num_layers=1):
        super(Embeddings, self).__init__()
        self.d_model = d_model
        self.dropout = nn.Dropout(0.1)
        self.embed = nn.Embedding(vocab_size, d_model)
